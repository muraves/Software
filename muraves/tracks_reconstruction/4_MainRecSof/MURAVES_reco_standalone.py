...
####################################################################################################################################
            ...
            def select_best_4p_index_cpp_style(
                intercept_4p: Sequence[Sequence[float]],
                cluster_c4_texp: Sequence[Sequence[float]],
                row_idx: int,
            ) -> tuple[int, int]:
                """Replicate C++ tie-break: first candidate with Texp>0, else index 0."""
                best_idx = 0
                is_texp_null = 1

                if row_idx < 0 or row_idx >= len(intercept_4p):
                    return best_idx, is_texp_null

                n_candidates = len(intercept_4p[row_idx])
                texp_row = cluster_c4_texp[row_idx] if row_idx < len(cluster_c4_texp) else []
                for k_idx in range(n_candidates):
                    texp = texp_row[k_idx] if k_idx < len(texp_row) else -1.0
                    if texp > 0:
                        is_texp_null = 0
                        best_idx = k_idx
                        break
            ...
            if tracks_xz.Track_3p_to_4p_index and tracks_xy.Track_3p_to_4p_index:
                ...
                if best_track_xy_ind in tracks_xy.Track_3p_to_4p_index and best_track_xz_ind in tracks_xz.Track_3p_to_4p_index:
                    ...
                    track_4p_idx_xy, best_track_4p_is_texp_null_xy = select_best_4p_index_cpp_style(
                        tracks_xy.intercept_4p,
                        cluster_c4_texp_xy,
                        track_3p_of_4p_idx_xy,
                    )

                    track_4p_idx_xz, best_track_4p_is_texp_null_xz = select_best_4p_index_cpp_style(
                        tracks_xz.intercept_4p,
                        cluster_c4_texp_xz,
                        track_3p_of_4p_idx_xz,
                    )

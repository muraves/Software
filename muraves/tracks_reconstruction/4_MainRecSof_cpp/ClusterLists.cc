...
uint32_t DeterministicSmearingRNG::next_u32() {
  state ^= (state << 13);
  state ^= (state >> 17);
  state ^= (state << 5);
  return state;
...
int DeterministicSmearingRNG::uniform_int(int low, int high) {
  const uint32_t span = static_cast<uint32_t>(high - low + 1);
  return low + static_cast<int>(next_u32() % span);
}
...
ClusterCollection CreateClusterList(vector <double> Deposits, const double EnergyThreshold_clusterStrip, const double EnergyThreshold_singleStrip, double AdStripsThEnergy_singleStripCl,double T_exp1,double T_exp2,vector<double>TriggerMask1, vector<double>TriggerMask2, DeterministicSmearingRNG* smearing_rng) {
  //EXTERNAL PARAMETERS  
  ...
  for(int st=0; st<nStrips; st++) {
	...
	if(cluSize>0) {
      ...
      if(smearing_rng != nullptr) {
        cluster_position = cluster_position + (double) (smearing_rng->uniform_int(range_from, range_to))/1000.;
      }
      else {

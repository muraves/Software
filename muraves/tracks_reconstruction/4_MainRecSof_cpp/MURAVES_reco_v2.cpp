...
int main(int argc, char* argv[]) {
  ...
  // Measure total wall time so the result is directly comparable with the Python version.
  auto start = std::chrono::steady_clock::now();
  double tree_fill_wall_time = 0.0;
  double output_finalize_wall_time = 0.0;
  ...
  run_ss >> run;
  DeterministicSmearingRNG* smearing_rng = nullptr;
  unique_ptr<DeterministicSmearingRNG> owned_smearing_rng;
  if(argc >= 4) {
    const uint32_t smearing_seed = static_cast<uint32_t>(strtoul(argv[3], nullptr, 10));
    owned_smearing_rng = make_unique<DeterministicSmearingRNG>(smearing_seed);
    smearing_rng = owned_smearing_rng.get();
    cout << "Using deterministic single-strip smearing seed: " << smearing_seed << endl;
  }
  char run_string[10];
  ...
  strcpy(SavingPath_miniTree,"/workspace/muraves_outputs/RECONSTRUCTED/");
  ...
  //strcpy(SavingPath_AnalysisTree,"~/muraves_outputs/RECONSTRUCTED/");
  ...
  strcpy(PATH_ADC,"/workspace/muraves_outputs/PARSED/");
  strncat(PATH_ADC,color,10);
  strncat(PATH_ADC, "/v0/", 5);
  ...
  strcpy(SlowControlFileName,"/data/RAW_GZ/");
  ...
  */
  ...
  if (!file_telescope.is_open()) {
    cerr << "ERROR: Cannot open file: " << telescopeConfig_name << endl;
    exit(1);
  }
...
int  NGooDTracks4p=0;
  ...
  resuts_p1x = CreateClusterList(Deposits_p1x,s1,s2,s3,Texp_1x1,Texp_1x2,TriggerMaskStrips.at(0),TriggerMaskStrips.at(1),smearing_rng);
  resuts_p2x = CreateClusterList(Deposits_p2x,s1,s2,s3,Texp_2x1,Texp_2x2,TriggerMaskStrips.at(4),TriggerMaskStrips.at(5),smearing_rng);
  resuts_p3x = CreateClusterList(Deposits_p3x,s1,s2,s3,Texp_3x1,Texp_3x2,TriggerMaskStrips.at(8),TriggerMaskStrips.at(9),smearing_rng);
  resuts_p4x = CreateClusterList(Deposits_p4x,s1,s2,s3,Texp_4x1,Texp_4x2,TriggerMaskStrips.at(12),TriggerMaskStrips.at(13),smearing_rng);

  resuts_p1y = CreateClusterList(Deposits_p1y,s1,s2,s3,Texp_1y2,Texp_1y1,TriggerMaskStrips.at(3),TriggerMaskStrips.at(2),smearing_rng);
  resuts_p2y = CreateClusterList(Deposits_p2y,s1,s2,s3,Texp_2y2,Texp_2y1,TriggerMaskStrips.at(7),TriggerMaskStrips.at(6),smearing_rng);
  resuts_p3y = CreateClusterList(Deposits_p3y,s1,s2,s3,Texp_3y2,Texp_3y1,TriggerMaskStrips.at(11),TriggerMaskStrips.at(10),smearing_rng);
  resuts_p4y = CreateClusterList(Deposits_p4y,s1,s2,s3,Texp_4y2,Texp_4y1,TriggerMaskStrips.at(15),TriggerMaskStrips.at(14),smearing_rng);
  ...
  auto tree_fill_start = std::chrono::steady_clock::now();
  tree->Fill();
  tree_fill_wall_time += std::chrono::duration<double>(std::chrono::steady_clock::now() - tree_fill_start).count();
...
// CALCULATE THE DURATION OF THE RUN ///////////////////////////////////////////////
  ...
  auto output_finalize_start = std::chrono::steady_clock::now();
...
/////////// PARAMETERS TO CONTROL THE ANALYSIS PERFORMANCIES --> FILLED IN THE MINITREE //////////
  ...
  tree_OnePhe->Write();
  file_OnePHe->Close();
  output_finalize_wall_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - output_finalize_start).count();
  ...
  double output_production_wall_time = tree_fill_wall_time + output_finalize_wall_time;
  double reconstruction_core_wall_time = elapsed - output_production_wall_time;
  if (reconstruction_core_wall_time < 0.0) reconstruction_core_wall_time = 0.0;
  cout << "Number of events: " << ev <<endl;
  cout << "[progress] Reconstruction core wall time: " << reconstruction_core_wall_time << " s" << endl;
  cout << "[progress] ROOT event fill wall time: " << tree_fill_wall_time << " s" << endl;
  cout << "[progress] ROOT finalize wall time: " << output_finalize_wall_time << " s" << endl;
  cout << "[progress] Output production wall time: " << output_production_wall_time << " s" << endl;
  cout << "[progress] Total wall time: " << elapsed << " s" << endl;
  if (ev > 0) {
    if (reconstruction_core_wall_time > 0.0) {
      cout << "[progress] Reconstruction core rate: " << (static_cast<double>(ev) / reconstruction_core_wall_time) << " ev/s" << endl;
    }
    cout << "[progress] Average processing rate: " << (static_cast<double>(ev) / elapsed) << " ev/s" << endl;
  }

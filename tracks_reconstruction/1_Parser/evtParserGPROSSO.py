...
skList = []
numEVT = sys.argv[2]
numSk = sys.argv[3] # board number =16
...
for ii in range (0,1):
	...
	if (CK == 0) and  ((linecounter-2)%int(numSk) == 0) :
		...
		offset = 0
		trigger = 0
		# Convert to UTC with timezone awareness
		s = inputF.readline()
		r = s.split('\t')
		timestamp_ms = r[0]
		...
		while s:
			s = inputF.readline()
			r = s.split('\t')
			#first data, strip number is 0, remove strip number, read only the electron vaule, convert to decimal
			h = int('0x'+r[1][2:], 16) 
			p[0+39*offset] = str(h) #voltage 
			p[1+39*offset] = r[0] # borad number 0-15
			h = int('0x'+r[2], 16)
			p[2+39*offset] = str(h)
			for i in range (3, 34): #until the 32 strip
				if (i <= 17):
					h = int('0x'+r[i][1:], 16)
					p[i+39*offset] = str(h)
				else :
			...
			h1 = int('0x'+r[34], 16) #0
			h2 = int('0x'+r[35], 16)
			#0 to identify the strip number in slave data 
			w1 = '{0:b}'.format(h1)
			w2 = '{0:b}'.format(h2)
			wh1 = ''
			wh2 = ''
			for i in range(16-len(w1)):
				wh1 = wh1 + '0' #make sure it is 16-bit binary numbers
			...
			p[38+39*offset] = tm # strip number combined when 1 binary
			
			#all 16 boards write in the same line 
